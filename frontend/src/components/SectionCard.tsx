...
const TYPE_OPTIONS: { value: Section["type"]; label: string }[] = [
  { value: "mcq",          label: "MCQ" },
...
export default function SectionCard({ section, index, onChange, onRemove, canRemove, dragHandleProps }: SectionCardProps) {
        ...
        <div className="gk-field">
          ...
          <div className="gk-stepper">
            <button type="button" className="gk-stepper__btn"
              onClick={() => update({ num_questions: clamp(section.num_questions - 1, 1, 50) })}
            ...
            <input
              ...
              min={1}
              max={50}
              onChange={(e) => update({ num_questions: clamp(parseInt(e.target.value) || 1, 1, 50) })}
            />
            <button type="button" className="gk-stepper__btn"
              onClick={() => update({ num_questions: clamp(section.num_questions + 1, 1, 50) })}
              disabled={section.num_questions >= 50}>+</button>
          </div>
        </div>
        <div className="gk-field">
