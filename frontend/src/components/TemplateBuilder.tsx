...
export default function TemplateBuilder({
  ...
  // ── Form state ──────────────────────────────────────────────────────────────
  const [name, setName] = useLocalStorage("tpl-name", "");
  const [subject, setSubject] = useLocalStorage("tpl-subject", "");
  const [grade, setGrade] = useLocalStorage("tpl-grade", "");
  const [difficulty, setDifficulty] = useLocalStorage<Difficulty>("tpl-difficulty", "medium");
  const [totalMarks, setTotalMarks] = useLocalStorage<number>("tpl-marks", 100);
  const [duration, setDuration] = useLocalStorage<number>("tpl-duration", 180);
  const [headingDetails, setHeadingDetails] = useLocalStorage("tpl-heading", "");
  const [instructions, setInstructions] = useLocalStorage("tpl-instructions", "");
  const [sections, setSections] = useLocalStorage<Section[]>("tpl-sections", [
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await templatesApi.create({ name: name.trim(), template });
        ...
        <div className="page-header">
          ...
          <div className="page-header__ornament">
            ...
            <div
              ...
              style={{
                background: "linear-gradient(270deg, var(--gold-border), transparent)",
              }}
            />
          </div>
