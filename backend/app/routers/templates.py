...
from app.models.db import Template as TemplateORM
from app.schemas.template import ExamTemplate, SaveTemplateRequest

logger = logging.getLogger(__name__)
router = APIRouter(prefix="/templates", tags=["templates"])

# Placeholder user until auth (Phase 4)
...
async def create_template(
    ...
    logger.info(
        "Template saved | id=%s | name=%s | subject=%s | grade=%s",
        template_id, body.name, body.template.subject, body.template.grade,
    )
...
async def list_templates(
    ...
    """Return all templates belonging to the current (placeholder) user."""
    result = await db.execute(
        select(TemplateORM)
        .where(TemplateORM.user_id == _PLACEHOLDER_USER)
...
async def _get_or_404(template_id: uuid.UUID, db: AsyncSession) -> TemplateORM:
    result = await db.execute(
        select(TemplateORM).where(
            ...
            TemplateORM.user_id == _PLACEHOLDER_USER,
        )
    )
