...
class GroqClient(LLMClient):
    ...
    async def generate(
        ...
        for idx, model in enumerate(models_to_try):
            ...
            while True:
                logger.debug(
                    "Groq request | model=%s | temp=%s | max_tokens=%d | sys_len=%d | usr_len=%d",
                    model, temperature, current_max_tokens, len(system_prompt), len(user_message)
                )
                try:
                    ...
                    # 3. Handle model not found or payload too large — fall back to next model
                    if any(err in exc_str for err in ["model_not_found", "does not exist", "Request Entity Too Large", "request_too_large"]):
                        if idx < len(models_to_try) - 1:
                            logger.warning(
                                "Groq model %s failed (%s) — falling back to %s",
                                model, exc_str, models_to_try[idx + 1]
                            )
                            break  # advance to next model
                        raise LLMProviderError("groq", f"All Groq models failed: {exc}", original=exc)
